...
function shortenLink($link, $filename) {
    $string = bin2hex(openssl_random_pseudo_bytes(10));
    $url = qrcdr()->relativePath().qrcdr()->getConfig('url');

    $linkClass = new Link();
    $createLink = $linkClass->create($string, $link, $filename);

    return [
        'url' => $url . '/link.php?link=' . $createLink,
        'id' => $createLink
    ];
}

function addImageToDatabase($id, $image) {
    $linkClass = new Link();
    $createLink = $linkClass->addImage($id, $image);

    touch("../svgs/" . $id . '.svg');
    file_put_contents("../svgs/" . $id . '.svg', $image);
...
case '#pdf':
    ...
    $file_name_array = explode(".", $pdf['name']);
    $file_name = $file_name_array[0];
    $file_extention = $file_name_array[1];

    if ($mime_type === 'application/pdf' && $file_extention === 'pdf') {
        $url = qrcdr()->relativePath().qrcdr()->getConfig('url');
        $new_name = $file_name . '_' . time() . '.' . $file_extention;
        move_uploaded_file($pdf['tmp_name'], __DIR__.'/../uploads/'. $new_name);

        $output_data = $url . '/pdf/' . $new_name;
    }
...
if ($output_data) {
    ...
    $filename = isset($_POST['file_name']) ? filter_var($_POST['file_name'], FILTER_SANITIZE_SPECIAL_CHARS) : null;
    $output_data = shortenLink($output_data, $filename);

    $backcolor = isset($_POST['transparent']) ? 'transparent' : $backcolor;

    // $optionlogo = $optionlogo && $optionlogo !== 'none' ? $optionlogo : false;
    $filename = $filename ? $filename : $PNG_TEMP_DIR.md5($output_data['url'].'|'.$errorCorrectionLevel.'|'.$matrixPointSize.time());
    $filenamesvg = $filename.'.svg';
    $basename = basename($filenamesvg, '.svg');

    $codemargin = $outerframe !== 'none' ? $frames[$outerframe]['frame_border'] * 2 + 1 : 2;
    $content = QRcdr::svg($output_data['url'], $filenamesvg, $errorCorrectionLevel, $matrixPointSize, $codemargin, false, $backcolor, $frontcolor, $optionstyle);

    addImageToDatabase($output_data['id'], $content);
